...
<script type="text/javascript">
    function update(caseid) {
        ...
        $.ajax({
            url: '<%=basePath%>user/message/addMessage',
            type: "post",
            data: form,
            /* 执行执行的是dom对象 ，不需要转化信息*/
            processData: false,
    ...
    function mark(nodeid, flag) {
        ...
        $.ajax({
            url: '<%=basePath%>user/node/mark/' + nodeid + '/' + flag,
            type: "post",
            dataType: 'json',
            data: {"markId": nodeid},
            success: function (d) {
    ...
    function showCaseNode(caseid, isedited) {
        ...
        if (flag == 1) {
            $.ajax({
                url: '<%=basePath%>user/nodeMap/case/caseid/' + caseid,
                type: "post",
                dataType: 'json',
                success: function (d) {
                    console.log(d);
        ...
        $.each(data.baseNodes, function (index, item) {
            ...
            if(isedited==1){
                marker.addEventListener("dblclick", function () {
                    //弹出具体点位的信息
                    window.open("<%=basePath%>user/node/" + this.id + "?nodetype=" + this.nodetype);
    ...
    function caseSearchPage(rel) {
        ...
        $.ajax({
            url: '<%=basePath%>user/nodeMap/case/' + rel,
            type: "post",
            data: form,
            /* 执行执行的是dom对象 ，不需要转化信息*/
            processData: false,
    ...
    function updateNodeTable(data) {
        ...
        if (data.baseNodes.length != 0) {
            $.each(data.baseNodes, function (index, item) {
                ...
                str += '<td class="col-sm-2">' +
                    '<button class="btn btn-info"  onclick="window.open(\'/user/node/' + item.nodeid +
    ...
    function nodeSearchPage(rel) {
        ...
        $.ajax({
            url: '<%=basePath%>user/nodeMap/node/' + rel,
            type: "post",
            data: form,
            /* 执行执行的是dom对象 ，不需要转化信息*/
            processData: false,
